...
# Shared flag for recording state
is_recording = False
...
def generate():
    global output_frame, video_lock, is_recording
    ...
    while True:
        ...
        if is_recording:
            ...
            # Yield a blank frame when not recording
            blank_frame = np.zeros((480, 640, 3), dtype=np.uint8)
            (flag, encodedBlankImage) = cv2.imencode(".jpg", blank_frame)
            if flag:
                yield(b'--frame\r\n' b'Content-Type: image/jpeg\r\n\r\n' + 
                    bytearray(encodedBlankImage) + b'\r\n')

def process_video():
    global video_capture, output_frame, video_lock, is_recording

    while True:
        if is_recording and video_capture.isOpened():
...
def start_recording():
    global video_capture, audio_stream, is_recording
    if not is_recording:
        is_recording = True
